...
public class SaleController {
    ...
    @GetMapping (path =  "/storeupnumber/{number}")
    public ResponseEntity<ResponseDTO> getStoresUpNumber(@PathVariable int number)
    {


        if( saleService.getStoresUpNumber(number).isEmpty()){
            return new ResponseEntity<>(new ResponseDTO(404,
                    "No hay ninguno que lo supere",null),
                    HttpStatus.BAD_REQUEST);}
        else {

            return  new ResponseEntity<>(new ResponseDTO(200,
                    saleService.getStoresUpNumber(number),null),
                    HttpStatus.OK);
